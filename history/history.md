...
- **feat(waveform)**: [2026-08-27] Implemented **Audio Waveform Generator (Avee Player-Style MP4 Visualizer) - Level 2** — Added a new Waveform Gen tab to the Audio Mode toolkit. Programmed dynamic generation of reactive video waveforms (`.mp4`) from audio files using FFmpeg's `showwaves` filter. Created config inputs for Waveform Style (cline, line, point, p2p), Video Resolution & Aspect Ratio (Landscape 16:9, Portrait 9:16, Square 1:1), and custom sliders for Waveform Height (50px-600px) and Vertical Position offset (0%-100%). Wired up a Background cover image picker using Tauri's native dialog API to loop and overlay the transparent sound waves over custom image covers at 80% opacity, defaulting to a solid black background color filter dynamically if no cover is selected.
- **feat(theme-icons)**: [2026-08-27] Extended **Windows XP Retro Icon Pack** to Right Panel & Console — Programmed dynamic injection of retro icons for right-panel headers, preview play control buttons, settings list items (Emotional Mode, Video Preview, Console Logs, YT Downloader), and the Console logs bento box header. Configured custom inline scaling rules (18px) for list/header items, remapped Console Logs to the Command Prompt icon, and hid the green pulsing active dot in Windows XP theme to present a fully cohesive classic terminal console header.
- **feat(theme-icons)**: [2026-08-26] Implemented **Windows XP Retro Icon Pack Integration** — Automated resizing and compression of 354 high-resolution PNG icons down to standard 48x48 pixels using a custom Python script (reducing folder size from 159MB to under 2MB). Programmed dynamic DOM injection of retro icons during DOMContentLoaded in `main.js` and designed clean CSS toggle rules in `style.css` to hide standard Lucide SVG icons and display custom 22px retro PNG icons only when the Windows XP theme (`theme-winxp`) is active, restoring default icons seamlessly when switching to other themes. Mapped specialized retro icons: Command Prompt for the batch processor, Hearts for the Lofi Cassette converter, and Windows Media Player 7 for the YouTube Downloader and switchers.
- **feat(audio-convert)**: [2026-08-26] Implemented **Advanced Audio Converter Settings & OPUS Support** — Enhanced the basic Audio Format Converter tool view with an expandable advanced settings panel. Added checkbox-toggle visibility switching using the design system's `.checkbox-group` classes and custom slide-down fade transitions. Integrated configurable parameters for Audio Bitrate (320kbps, 192kbps, 128kbps, 96kbps), Channels (Stereo/Mono), and Sample Rates (48kHz, 44.1kHz, 32kHz, 22.05kHz). Programmed dynamic FFmpeg command generation in `main.js` that maps parameters (`-b:a`, `-ac`, `-ar`) based on settings, automatically skipping bitrate limits for lossless outputs (`wav`, `flac`) to preserve format integrity and adding full container conversion support for the modern OPUS format.
- **feat(video-thumbnail)**: [2026-08-26] Implemented **Custom Video Thumbnail Embedding** — Added a dedicated "Video Thumbnail" tab under the Video Mode toolkit sidebar and created a glassmorphic editor panel. Wired up a file picker dialog restricted to image files (`.png`, `.jpg`, `.jpeg`) to browse covers and connected the save action to compile and execute a zero-latency stream-copying FFmpeg command (`-i video -i image -map 0 -map 1 -c copy -disposition:v:1 attached_pic -y output`). Registered dynamic click and success mascot dialogues for Aura across Hinglish, English, Sarcastic, Hacker, and Lazy dialects. Modified focused preview mode styling by restricting layout overrides to `:not(:fullscreen)` to ensure video centering and full-screen scaling functions perfectly when expanding.
- **feat(focused-preview)**: [2026-08-25] Implemented **Focused Large Preview Mode with Dynamic DOM Relocation & Eyesight Protection Timer** — Developed a dynamic layout transition system that monitors user activity inside visual tool tabs (`trim`, `split`, `rotate`). If active for more than 3 minutes, the mascot Aura triggers a protective dialogue (registered across Hinglish, English, Sarcastic, Hacker, and Lazy dialects) and shifts the video preview card (`#live-preview-card`) from the right panel to the center column inside `.scrollable-content` above the tool controls for a much larger, clear view. Added custom CSS rules for `.focused-preview-active` with full-width stretching, expanded height (`260px` to `350px`), and a smooth slide-down animation while keeping audio and video playback continuous during layout transitions. Bypassed web browser context crashes on load by implementing mock environment Tauri `invoke`/`listen` fallbacks. Refined layout positioning on [2026-08-26] by relocating the expanded preview card below the file browser bento (`.global-controls`) and above active tool views, automatically hiding the CRT monitor overlay (`#crt-monitor-overlay`) to allow borderless full-video scaling in retro themes, and adding compact style overrides for visual tools (hiding descriptions, shrinking padding/margins) to avoid vertical scrollbars.
- **feat(theme & fix)**: [2026-08-19] Polish theme controls and fix video stabilization & YouTube downloader — Redesigned Control Panel UI with solid dark theme sidebar and custom miniaturized active preview cards. Overrode styling for Windows XP (Luna) and Windows 98 retro themes, including custom mode switcher sliders, color schemes, and layouts. Set about footer text to custom retro colors (dark green for XP, black for 98) while keeping modern themes readable. Fixed video preview inside the CRT monitor frame by increasing height from 30% to 58%. Corrected video stabilization in FFmpeg by replacing the invalid parameter 'shake' with 'shakiness' and implemented path escaping for Windows paths containing spaces. Upgraded YouTube Downloader with format/resolution selections, automated cover art thumbnail embedding, and bypassed 403 Forbidden errors by using the Android player client.
- **fix(perf)**: [2026-07-30] Resolved **UI Startup Freeze & System Metrics Release Crash** — Refactored command-spawning Tauri handlers (`get_video_duration`, `generate_thumbnail`, `list_videos_in_folder`, `get_ffmpeg_status`, `check_ffmpeg`, `get_ffmpeg_version`, and `get_ytdlp_status`) to asynchronous `async fn` execution. Moved CPU and RAM metrics polling into a dedicated, low-priority background thread `start_system_monitor()` that updates global atomic variables once per second. Updated `get_system_metrics` to a lightweight, synchronous call that instantly reads and returns these atomic values without any sleeping or large struct heap/stack allocations. This resolves UI startup freeze, eliminates the runtime micro-stuttering, and completely prevents the Windows stack overflow crash (`0xc00000fd`) in release builds.
- **feat(theme & visualizer)**: [2026-07-28] Added **Synthwave DeLorean Sunset Background & Custom Sound Visualizer** — Set up a custom, blurred and dimmed background overlay (`theme-asset-retro/Screenshot 2026-07-27 153551.svg`) with enhanced glassmorphic panel opacity for the Synthwave theme. Fixed mode switcher buttons borders and backgrounds clashing with the sliding pill indicator. Developed a new custom sound visualizer preset **"DeLorean Synthwave Sunset"** in HTML5 Canvas, featuring a sliced neon sun pulsing to bass, an infinite scrolling 3D wireframe grid accelerating on beats, silhouette mountains reacting to mids, and twinkling space stars reacting to treble.
- **feat(downloader)**: [2026-07-20] Relocated **YouTube Downloader Mode Toggle & Mascot Hover Dialogues** — Migrated the YouTube Downloader from a hidden sidebar tab in Audio Mode to a global toggle inside the **Modes & Preview** settings card in the right info panel. Toggling it ON switches the central panel to the spacious YouTube Downloader form view, while toggling it OFF restores the previously active tool view. Added conditional mascot hover dialogues (`interact_ytdlp_hover_logs_on` and `interact_ytdlp_hover_logs_off`) across all 5 dialects depending on whether the Console Logs toggle is enabled. Created folder containers under `src/emotive-ani-voice/`.
